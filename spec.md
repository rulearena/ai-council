...
97. **AIDLC Workflow 首次採用（AIDLC bootstrap）**：中央工作流 repo（AIDLC Workflow，canonical identity 見 `docs/agents/workflow-bindings.md`，固定 revision `de1aca49181af3ce8583f6d59b034ea8136d42c6`）正式成為本專案流程來源。新增 `docs/agents/workflow-bindings.md`（§0 來源 bootstrap、§1 專案資訊、§2 三角色執行者與模型、§3 六個正式流程入口、§4 canonical 紀錄、§5 安全邊界、§6 交接）；`AGENTS.md`／`CLAUDE.md` 政策入口改為 bootstrap 單一語意並修正過時 `.scratch` tracker 說明；三份角色 prompt 必讀清單加入 bootstrap 並修正與中央規範的 3 處衝突（orchestrator「可直接動手的範圍」違反中央「不實作」、reviewer Verdict 缺 `Reviewed identity`、必讀順序缺綁定檔）。review(doc) 由獨立 Reviewer subagent 對 fixed range `83cea96...96e083c` 通過（1 Minor＋1 Nit：本地 `multi-agent-development.md` 與中央同相對路徑的優先順序、Idea 入口路徑，均已套用修正並複審 `pass`），Orchestrator fast-forward merge 至 main（`96e083c`），worktree/branch 已清理、既有 untracked 保留。三角色模型經 Human Owner 確認皆為 session runtime `opencode/big-pickle`（綁定檔 §2）。**2026-07-31 Human Owner 驗收通過，`accepted / done`。**
...
**#103 Slice 3 follow-up Executor continuation（2026-08-10）**：固定 prior HEAD `64846e5156dca1dea01e2fc1d63903c76f9a3794`，新增 implementation commit `36f8a6cba6c3e73a32c6711e301a966f54531f6e`（continuation range `64846e5..36f8a6c`）。修正明示但非 boolean 的 `host_acl_explicit` marker fail-closed、剩餘 budget 小於單段時仍保留實際 segment identity／bounded prefix、引用 chip reader 以 exact source target 捲動並在 meeting switch 清除 stale target；補上 API no-# blob-read spy／body isolation、@all partial-visibility atomic envelope、large-source non-empty retrieval、runner invalid-citation parse diagnostics 與 public citation deletion/unavailable Playwright test。驗證：新增 focused backend **22 passed**（source **11**、API matrix **6**、runner citation **5**）；frontend unit **178 passed**；`npm run build` 綠；`npx playwright test tests/e2e/chatroom.spec.ts --list` 列出 **55 tests**（含 13.10c 與 13.10d），但 Chromium binary 不存在故 focused Chromium 未執行。一次 bounded relevant backend aggregate 為 **330 passed／1 failed**；失敗為既有 directed retry timing flake，隔離重跑 **1 passed**，未修改該 retry 行為。OpenSpec strict 與 `git diff --check` 綠；尚未獨立 Gate B，未宣告 Gate B 或 Human acceptance。Task 3.8 已依「可執行非 Chromium layer 全綠且 E2E scenarios 存在、明示 Chromium 未執行」標記完成；3.7 仍未標記完成，待 public reader/unavailable browser tests 實際綠燈。
**#103 Slice 3 Gate-B fix Executor continuation（2026-08-10）**：承接 failed Gate-B HEAD `10dde9a51f02b3f3943fca8dfd892b6280522e58`，approved base `521d6c1398da449ed1fa2920aae62cb49e5785d7`。修正 retry wait race；以單一 request-wide budget 同時計入 required labels/layers、instruction、quote、recent transcript 與 source excerpts，舊 transcript 先淘汰且 residual 為零時不強行配置 source；新增 same-label stable presentation discriminator；citation chip 以 authoritative `reader_ref` 開啟 exact attachment/evidence reader，close、meeting switch、deleted/inactive unavailable 均有覆蓋。focused backend **41 passed**、frontend unit **180 passed**、build 綠；installed Chrome channel focused `13.10c` **1/1**、`13.10d` **1/1**；OpenSpec strict、`git diff --check` 綠。仍未 Gate-B re-review、merge 或 Human acceptance。

**#103 Slice 3 Gate-B findings 修正（2026-08-10，Executor，尚待獨立複審）**：以 failed Gate-B HEAD `7c141f52df7891e69322deae0429ca3d330cdb2c` 為起點，未 merge／未宣告 Gate B 或 Human acceptance。新增並通過實際 adapter 收到之 canonical `system`／`developer`／`user` prompt budget regression；request-wide allocator 以各 fanout target 的固定 rendered layers 取最大 reservation，固定層飽和時 source residual 為零，source snapshot／retry identity 維持 frozen。精準恢復 non-chatroom directed-response 等待 2 events，保留 directed-retry 等待 terminal 第 3 event。修正 unloaded evidence citation reader 等 materials authoritative load 後開啟、fail-closed unavailable、close parent ack；同一 mount close 後再次點擊同一 citation 可重開 exact content。Chrome channel 實跑 `13.10c`／`13.10d`／`13.10e` **3/3 passed**；backend focused suites **45 passed**，broader API directed/budget/chatroom invocation **24 passed**；frontend unit **180 passed**、production build、OpenSpec strict、diff check 通過。仍未 Gate-B re-review、merge 或 Human acceptance；本回合 tracked implementation commit 尚待建立。

**#103 Slice 3 ACL finding 修正（2026-08-10，Executor，尚待獨立複審）**：固定起點 `31a535a37bae0b86c8ddc59dd4fe9833c4ee03fc`、approved base `521d6c1398da449ed1fa2920aae62cb49e5785d7`。只修 `host_acl_explicit` presence/value serialization：MaterialVersion 分開保存欄位是否存在與其 value，明確存在的 `null`／`false`／字串／`0`／`1` 經 persistence→projection 皆 fail-closed，不把 Host 加入 `visible_roles`；Host source request 在 event/job 前回 `400 INVALID_SOURCE_REF`，legacy 欄位真正缺席仍保留 fallback。新增 API matrix **6 passed**（含 legacy control）；source/case focused **24 passed**、isolated runner/prompt/context **1 + 85 passed**；frontend unit **180 passed**、production build、OpenSpec strict；installed Chrome channel `13.10c`／`13.10d`／`13.10e` **3/3 passed**。未執行 full backend、未作 Gate B re-review、未 merge、未宣稱 Human acceptance；本回合 tracked implementation commit 待建立。

**#103 Slice 3 final Gate B／main integration（2026-08-10）**：獨立 Reviewer 對完整 exact range `521d6c1398da449ed1fa2920aae62cb49e5785d7..c8b13582ac260b7aaa304b13ac5330f8d162adbe`（8 commits）終局判定 `pass`、無 findings；前兩輪發現的完整 prompt budget、2／3-event regression、unloaded evidence reader、same-page close/reopen，以及明確 `null` ACL marker 越權均已修正。Reviewer 自建 ACL matrix 驗證 object／array **4 passed**、`null`／`false`／字串／`0`／`1` **7 passed**、legacy absence／literal true／new evidence／attachment mirror **3 passed**，另驗證 fixed-layer saturation **1 passed**；backend source/case **24 passed**、runner **17 passed**、source lifecycle/API **13 passed**、精確 2-event／3-event／full-budget **3 passed**，frontend unit **180 passed**、production build、installed Chrome `13.10c`／`13.10d`／`13.10e` 各 **1 passed**、OpenSpec strict 與 diff check 全綠。main 已 fast-forward 至 `c8b13582ac260b7aaa304b13ac5330f8d162adbe`；post-merge main 再驗證 backend **41 + 13 passed**、frontend unit **180 passed**、production build、OpenSpec strict 與 diff check。未執行 full backend、full Playwright 或額外人工瀏覽器 smoke。**Slice 3 狀態：`implemented / awaiting Human Owner acceptance`；Slice 4 shared memory 與 tasks 5.1–5.4 尚未實作，#103 整體仍在進行且不得 archive。**

**#103 invalid mention／raw-vs-selected-token acceptance milestone（2026-08-11）**：本 milestone 驗收的 exact implementation identity 為 `52449bbcd21770c858a84f7cb70a7f59ba428cff...fe5fca37cc8b527deeb9f1ad64adae8027dcb2c0`，commit chain 為 `4de2d3b`、`c9a5d9f`、`fe5fca3`；final Gate B 由兩個 fresh、互相獨立的 Reviewer sessions 分別執行 Standards 與 Spec 軸，兩軸皆 `pass`、無 findings。Executor merge 前證據為 frontend unit **190/190**、backend Unicode parity **8/8**、direct `vue-tsc`、Vite production build、installed Chrome regression `13.2a`／`13.2b`／`13.2c`／`13.2d`／`13.10f`／`13.10g`／`13.10h` **7/7**，以及 raw-`#` ordinary POST 回 **202** 且 payload 無 source authorization；Orchestrator 在 exact `fe5fca3` post-merge 驗證 composer unit **20/20**、backend routing/source suites **52/52** 與 `git diff --check`。

Human Owner 於 2026-08-11 在 exact app commit `fe5fca3` 的本機 AI_Council UI，以其已安裝的 Google Chrome 人工驗收；當次未保存 Chrome 的精確瀏覽器版本，故不推定或補造版本。Human Owner 原始回報為：直接貼上 `😀 @顧問 我們 #待辦總覽.md 本週有什麼需要完成的？` 後，訊息保留且未送出，畫面顯示可操作的 `@顧問` 尚未選取提示；從 `@`／`#` 候選選單選取 token 後可正常送出，並明確表示：「這個驗收可以通過。」人工證據僅涵蓋上述 paste retain/block＋actionable `@顧問` guidance 與候選選單送出；raw `#` 是普通文字且不授予附件正文注入權限（只有選取並驗證通過的 `#` source token 才能注入正文）、@all warning precedence、含 `@` 的 selected source span，以及 code-point／boundary regressions，均只列為自動化證據，未宣稱 Human Owner 人工驗證其內部授權或 edge cases。本 milestone 狀態為 **`accepted`**；**#103 整體仍為 `in progress`，OpenSpec change 維持 active，tasks 4.1–4.6 與 5.1–5.4 保持未勾選，不 sync／archive；下一階段為 Slice 4 shared memory／summary，之後才是 integration tasks。**
