...
**#103 Slice 2 implementation（2026-08-09）**：在固定 approved base `825a640dda02e815329b6017850673a84936b9e3` 的獨立 Executor worktree 實作 prompt layering 與固定 Persona：五個 chatroom role 載入必須同時具備 `persona_summary`／backend-only `persona_prompt`，`GET /modes`／participant projection 僅公開 summary；新聊天室 explicit roster 強制 Host、空 roster 維持全五角色、legacy 非空 subset read-time 僅補 Host，建立後 settings 只接受 active role model IDs。Chatroom model request 固定保存 `system`→`developer`→`user` 三層與 deterministic labeled flattening；OpenAI／Anthropic／Gemini native/fallback、CLI legacy flattening、mock observability 均保留正式模式的 absent-messages legacy payload。Slice 2 chat prompt 僅含 Persona、當輪 instruction、quote/recent transcript 與 message-only schema，chat endpoint 不再開啟或注入案卷／evidence／note body；`attachment_refs`、`#` source retrieval/citations、Host ACL marker、shared summary 仍分別保留給 Slice 3／4。證據：Slice 2 targeted backend **9 passed**、既有 chatroom runner **13 passed**、adapter/catalog aggregate **70 passed**、chatroom API regression **13 passed**、frontend unit **172 passed**、production build 綠；完整 backend bounded run 在既有 startup-health timing baseline failure 停止，非本變更錯誤；frontend Chromium E2E 本輪未執行。Implementation branch／exact HEAD 待 commit 後交 Gate B；狀態：**`implemented / awaiting Gate B review`**。
