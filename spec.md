...
**#103 Slice 2 implementation／Gate B fixes（2026-08-09）**：固定 approved base `825a640dda02e815329b6017850673a84936b9e3` 的獨立 Executor worktree 已實作 prompt layering 與固定 Persona：五個 chatroom role 載入必須同時具備 `persona_summary`／backend-only `persona_prompt`，`GET /modes`／participant projection 僅公開 summary；新聊天室 explicit roster 強制 Host、空 roster 維持全五角色、legacy 非空 subset read-time 僅補 Host，建立後 settings 只接受 active role model IDs。Chatroom model request 固定保存 `system`→`developer`→`user` 三層與 deterministic labeled flattening；OpenAI／Anthropic／Gemini native/fallback、CLI legacy flattening、mock observability 均保留正式模式的 absent-messages legacy payload。Slice 2 chat prompt 僅含 Persona、當輪 instruction、quote/recent transcript 與 message-only schema，chat endpoint 不再開啟或注入案卷／evidence／note body；`attachment_refs`、`#` source retrieval/citations、Host ACL marker、shared summary 仍分別保留給 Slice 3／4。首次 Gate B 對 exact HEAD `138a47d780126f27bbe69fa978b7f9d231532727` 回覆 `needs-fixes`；本輪 Executor 已修正三項 Major：single-target Host／非 Host 都使用當前 mode 的固定 Persona 且空 Persona 在 transport 前拒絕、mention routing 與 meeting API 共用同一 active participant projection、NewCaseModal 以可操作 checkbox 與實際 create payload 覆蓋 frozen subset roster。修正後證據：finding-focused runner／audit **23 passed**、chat HTTP／legacy roster **14 passed**、relevant backend **366 passed**、frontend unit **172 passed**、production build、Chromium roster regression **1 passed**、OpenSpec strict validation 與 `git diff --check` 通過。尚未取得複審 Verdict，也未宣告 Human acceptance；狀態：**`implemented / awaiting Gate B re-review`**。
