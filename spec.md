...
**#103 Gate A 契約補充（2026-08-07）**：角色引用使用 display-name chip（`@顧問`／`@主持 AI`／`@全部角色`），chip 內隱藏 stable `role_id`；request 使用 code-point span token model：`mentions[{token_id,role_id,display_text,start,end}]`、`source_tokens[{token_id,source_ref,display_text,start,end}]`、ordered/deduped `source_refs`、`quoted_event_id`。手打未被 chip span 覆蓋的 role-like token 回 `INVALID_MENTION_TOKEN`；span、schema、source、quote validation 與 stale payload 均使用 OpenSpec 固定 400 envelope，rejection 先於 event/job。202／400／404／409 response envelope 與 warning code 依 OpenSpec 定義。source namespace 僅 `attachment:<file_id>` 與 `evidence:<evidence_id>`，排除 `notes`；citation 使用 selected allow-list 的 `{source_ref,label,segment_refs}` 並驗證 projection label／available segment refs。summary 可由 `POST /meetings/{meeting_id}/chat/memory/regenerate` 觸發，並以 `chatroom_memory_updated` projection event 更新；失敗保留上一版或標為 `empty`，不產生 feed event。OpenSpec artifacts fix commit 為 `a14ada0` 的後續修正版，仍須獨立 Reviewer 重新 Gate A。
