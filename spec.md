...
91. **無流程限制的 AI 聊天室模式**：建立 meeting 時只需聊天室名稱，不強制 AI 最終目標或固定 round／sequence；主席訊息是對話的驅動來源，可用 `@角色` 指定一位、`@all` 同時邀請全部角色，並可引用另一位角色的發言請大家評論。回應應採一般聊天的精簡篇幅，而非每位角色預設輸出長篇正式報告。附件與長期資料仍需沿用版本化案卷／證據契約，不得另存 API key 或建立不可追溯的臨時附件。需先釐清 mention parsing、parallel directed fanout、對話 context/token budget、回應長度政策與自由聊天是否需要可選 goal；本項是 #90 完成後的獨立產品批次。**OpenSpec artifacts 階段**：Human Owner 於 2026-07-20 核准進入 OpenSpec proposal → specs → design → tasks 規劃階段，等待 Codex Gate A review。
