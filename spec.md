...
**#103 invalid mention／raw-vs-selected-token acceptance milestone（2026-08-11）**：Human Owner 明確確認可通過本次驗收：直接貼上 `😀 @顧問 我們 #待辦總覽.md 本週有什麼需要完成的？` 時，未被選取的 `@顧問` 會保留在訊息中但不送出，並顯示可操作提示說明 `@顧問` 尚未選取；透過 `@`／`#` 候選選單選取 token 後可正常送出。`#` 的契約保持為：未選取的 raw `#` 只是普通文字，不授予附件正文注入權限；只有已選取且驗證通過的 `#` source token 才能注入附件／證據正文。@all warning precedence，以及含 `@` 的已驗證 source span，保留為自動化 regression evidence；不宣稱 Human Owner 逐項人工測試了這些自動化情境。本 milestone 狀態為 **`accepted`**；不代表 #103 整體或 Slice 4 已完成。**#103 整體仍為 `in progress`，OpenSpec change 維持 active；下一實作階段為 Slice 4 shared memory／summary，之後才是 integration tasks 5.1–5.4。**
