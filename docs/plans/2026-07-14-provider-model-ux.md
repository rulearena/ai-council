...
3. Ticket 03：test feedback/race red→green，最後統一視覺與可存取狀態。
4. Orchestrator 執行 full backend/unit/build/e2e 與真瀏覽器 smoke。
5. Standards/Spec 兩位獨立 Reviewer 平行審查；Blocking/Major 回原 Executor 修復後複驗。
6. 通過後更新 spec/HANDOFF/tickets，merge main，清理 worktree/branch/runtime。
