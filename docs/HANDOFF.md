...
- **角色分工（2026-07-20 起）**：OpenCode 是預設 Implementer／Integrator，負責 artifacts、隔離 worktree、TDD、review 修正、完整 gates、merge 與清理；Codex 是預設 review-only Reviewer，只做 artifacts 與 implementation 兩階段獨立審查，不改檔、不補 patch、不 merge。完整規範與可直接使用的 prompts 見 `docs/agents/multi-agent-development.md`、`docs/agent-prompts/`。
