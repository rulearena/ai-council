...
def test_planner_fails_safe_when_fixed_prompt_cannot_fit() -> None:
    ...
    assert raised.value.code == "CHATROOM_PROMPT_TOO_LARGE"


def test_planner_rejects_selected_source_when_no_actual_segment_can_fit() -> None:
    def build_snapshot(char_budget: int) -> dict:
        has_segment = char_budget > 0
        return {
            "selected_source_snapshot": {
                "schema_version": "chatroom-source-context/v1",
                "source_refs": ["evidence:source-1"],
                "sources": [{
                    "source_ref": "evidence:source-1",
                    "label": "來源",
                    "available_segment_refs": ["paragraph:0001"] if has_segment else [],
                }],
            },
            "source_excerpts": ["漢"] if has_segment else [],
        }

    with pytest.raises(ChatroomPromptPlanError) as raised:
        plan_chatroom_prompt(
            request_budget_tokens=500,
            target_role_ids=["host"],
            source_refs=["evidence:source-1"],
            selected_sources=[{"source_ref": "evidence:source-1"}],
            active_events=[],
            ...
            instruction="use the source",
            quoted_event_id=None,
            prompt_budget_for=lambda _role, _transcript, _allow_list: 499,
            prompt_tokens_for=lambda _role, _transcript, source_context, _allow_list: (
                501 if source_context else 499
            ),
            build_context=lambda *_args: "",
            build_snapshot=build_snapshot,
            source_allow_list_for=lambda sources: ",".join(
                ref
                for source in sources
                for ref in source.get("available_segment_refs", [])
            ),
        )
