...
- Base: `02f3b96`
- Test: `frontend/tests/e2e/control-flow.spec.ts` — `model select explains running lock and preserves idle selection hint`
...
npx playwright test tests/e2e/control-flow.spec.ts \
  -g 'model select explains running lock and preserves idle selection hint' --project=chromium
```
