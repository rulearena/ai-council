...
- *Behaviour change*: the in-rail model select saves immediately on selection (PATCH participant-models), so there is no intermediate dirty state that would block navigation. The previous drawer-based flow required an explicit "Save" button and could block navigation while dirty. The test `a delayed assignment save remains scoped to its meeting` was renamed from `…blocks leaving and remains scoped…` to reflect that navigation blocking no longer applies. Model assignment fallback warnings (deleted model) now display in the role rail via a banner below the seat list, replacing the removed drawer-level validation.
