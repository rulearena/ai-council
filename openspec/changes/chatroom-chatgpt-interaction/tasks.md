...
- [ ] 1.3 Implement the exact request schema: `content`, `mentions[{token_id,role_id,display_text,start,end}]`, `source_tokens[{token_id,source_ref,display_text,start,end}]`, ordered/deduplicated `source_refs`, and `quoted_event_id`; validate Python/Unicode code-point half-open spans, unique IDs, exact content slices, prefixes, bounds, overlap, and source_refs equality before routing. Use the complete 400 code set `INVALID_REQUEST_SCHEMA`, `INVALID_MENTION_TOKEN`, `STALE_MENTION_PAYLOAD`, `MENTION_TOKEN_MISMATCH`, `INVALID_SOURCE_REF`, `STALE_SOURCE_PAYLOAD`, `SOURCE_TOKEN_MISMATCH`, `SOURCE_NOT_VISIBLE_TO_TARGET`, `SOURCE_NOT_READABLE`, plus exact 404/409 envelopes.
- [ ] 1.4 Make the backend token validator authoritative: scan uncovered role-like @ tokens, apply normal `INVALID_MENTION_TOKEN` vs all-chip warning precedence, preserve original content plus token metadata in the human event, remove verified chip spans from normalized prompt instruction, and route every send through chatroom path.
...
- [ ] 1.8 Add direct API/UI tests for exact accepted/rejected schemas and status codes, missing/wrong/unknown fields, blank content, malformed quote, well-typed missing quote graceful ignore, malformed/non-overlap spans, stale mention/source payloads, every 400 code above, exact 404/409 envelopes, no-event/no-job side effects, raw role-like @, email, same-label chip vs hand-typed text, duplicate chips, all-warning precedence, and `#` chip vs hashtag.
...
- [ ] 3.3 Implement the `chatroom_sources` projection/listing for `attachment:<file_id>` uploads plus `evidence:<evidence_id>` legacy/initial active text evidence; exclude notes; expose authoritative source_ref/label/kind/readable/active/reader_ref/available_segment_refs; use existing readers, Host evidence fallback, and whole-request validation for every multi-role/all target.
