...
For an old chatroom meeting with no participant snapshot or an empty snapshot, read-time projection preserves the historical compatibility default of all five roles. For a non-empty stored subset that predates Host, it appends Host and resolves its model through the existing meeting-default/fallback assignment path while preserving only the stored member subset. It never rewrites metadata or historical events. This avoids migration while preventing intentionally omitted catalog roles from entering `@all` or source authorization.
...
The frontend preserves first-selection order and deduplicates repeated `source_ref` values. The backend resolves each source ref within the meeting, checks active state and target-role visibility, then applies the closed kind matrix. Every evidence version newly written after this capability lands—including initial evidence, later versions, and chat-upload mirrors—persists `host_acl_explicit: true`; `true` is the only accepted present value. Absence is the durable discriminator for a pre-Host legacy version and is never backfilled. Host may use the legacy fallback only when the active version omits this marker. When the marker is true, Host follows `visible_roles` exactly and is denied if absent.

An `attachment:<file_id>` is readable only when its active blob has a `.txt` or `.md` extension. If it has a linked evidence ID, visibility and active state—including `host_acl_explicit` behavior—come from that exact active evidence version; an unlinked legacy attachment has meeting-wide visibility to the active roster because attachment events have no role ACL. An `evidence:<evidence_id>` is readable when its active version contains a non-empty string and follows the marker-aware `visible_roles` rule, without an extension check. For multi-role and `@all`, any target/source failure rejects the whole request. Existing data is projected at read time without migration.
