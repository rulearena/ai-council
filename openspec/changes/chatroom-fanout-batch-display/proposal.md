...
- `frontend/src/composables/useCouncil.ts`: chatroom mention send already queues every mentioned role via `pendingRoles`; verify per-role completion clears each member without collapsing the whole round.
