...
- [ ] 2.2 Merge with arrived members: round expected set = captured queuedRoles ∪ roles seen via `chat-fanout-*` events for that round; round N = `|expected set|`, fixed once known, never growing with arrivals (reconnect/reload degradation → arrived-members-only, never over-counting)
