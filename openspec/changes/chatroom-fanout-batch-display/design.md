...
- **Alignment**: the captured set SHALL be aligned to the meeting's participant roles (the same set the backend can fan out to — the backend silently drops mentioned roles without a model assignment, api.py:2525-2527), so N never includes a role that can never emit an event. Roles outside the participants never enter the expected set.
...
- **Terminal convergence**: the round reaches terminal when every expected member has completed or failed. If, despite alignment, an expected slot never emits an event (e.g. backend dropped it), that slot SHALL NOT block the round from reaching terminal once all other expected members are done — progress counts it as unknown rather than pending forever.
...
3. **Terminal** (every expected member completed or failed, per D3's terminal convergence: an expected slot that never emitted an event counts as unknown and does not block terminal): header shows a completion label (`全部 N 位已回應`) or a partial label if any slot failed; an unknown slot keeps the header at its pending `x/N` (never claiming "全部 N 位已回應"); no slot remains thinking.
...
`applyPendingRoleUpdates` currently clears the entire `pendingRoles` on any member failure (useCouncil.ts:787-790). That rule is justified for serial relay (backend halts remaining steps after the first failure) and harmless for parallel batches (each member is an independent future and keeps running, but parallel thinking is event-driven via `parallelRoundState` so the full-clear has no practical effect). It is **wrong for chatroom fanout**: a failed fanout member does not stall the others — they run independently and must keep thinking. For chatroom fanout rounds, failure handling must change so a failed member:
