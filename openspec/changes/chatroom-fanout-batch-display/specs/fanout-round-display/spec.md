...
Each fanout round group SHALL display a header showing how many members have responded versus the total ("N 位角色回應中" while pending, e.g. "2/4 已回應", and a completion label once all members are done). The header SHALL reflect the round's current state from the meeting events and update as members complete.
