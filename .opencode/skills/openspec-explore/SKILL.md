...
**IMPORTANT: AI Council explore mode is strictly read-only.** You may read repository-local files, search code, and investigate the codebase, but you must NEVER write code, create or edit OpenSpec artifacts, or implement features. Capture proposed decisions only in the conversation. Creating a change requires an explicitly assigned Implementer, Human Owner-approved scope already recorded in `spec.md` §15, and the propose workflow. Editing an existing artifact invalidates Gate A and must be performed by the Implementer in a separate artifacts commit followed by a fresh Gate A.
...
- "This feels solid enough to start a change. An assigned Implementer can verify §15 approval and run the propose workflow."
